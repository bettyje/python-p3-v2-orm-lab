...
class Review:
    ...
    #     if not isinstance(value, int):
    #         raise ValueError("Employee ID must be an integer.")
    #     # Assuming Employee is a class with a method to check if an ID exists
    #     if not Employee.exists(value):
    ...
    def instance_from_db(cls, row):
        ...
        if row[0] in cls.all:
            ...
            review = cls(year=row[1], summary=row[2],
                         employee_id=row[3], id=row[0])
